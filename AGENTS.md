...
└── MasanielloApp\             soluzione .NET (C#, net10.0-windows)
    ...
    │   │                      auto-rilevamento colonna numeri, fallback HTML)
...
- Caricamento da Config (xls/xlsx/csv/txt) → tabella `permanenze` (numeri CSV,
  ...
  scarta colonne indice/progressive (anche con buchi, es. "Boule"), colonne costanti
  e flag 0/1, sceglie il foglio con più numeri e usa l'intestazione "Numero" come
  indizio. Formato verificato = export laroulette.it permanenzimetro (il file 888
  ha quel formato: Boule|Numero|Rosso|Nero|…|D1-D3|C1-C3). Se un .xls/.xlsx non è
  un vero Excel (tabella HTML rinominata) ricade sulla lettura testo/HTML.
...
- Config "Gettone minimo (€)" = gettone più piccolo utilizzabile sul tavolo (max 2
  decimali): ogni posizione riceve multipli interi del gettone e la puntata minima
  è 5×gettone (S1) o 11×gettone (S2); la Config la mostra live.
